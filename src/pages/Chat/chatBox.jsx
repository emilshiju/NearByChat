...
const ChatBox = () => {
  ...
  const showToastMessage = (messageTodisplay) => {
  ...
  useEffect(() => {
    getAllConversation(userId).then((res) => {
      setSideMessage(res);
    });
  }, [messages]);
  ...
  const updateSingleChat = () => {
    if (receiverId) {
      api.post("/getSingleChat", { receiverId, userId }).then((resposne) => {
        console.log(resposne.data.profile);
        if (resposne.data.chat) {
          resposne.data.chat.map((a) => {
            console.log(a);
          });
          const array = resposne.data.chat;
          setMessages(array);
        }

        setOppositeOne(resposne.data.profile);

        const member = resposne.data.chatroom?.members?.find(
          (a) => userId === a.userId
        );
  ...
  const sendMessage = () => {
    if (!chatRoomId) {
      if (receiverId) {
        api.post("/createChatRoom", { receiverId, userId }).then((res) => {
  ...
  useEffect(() => {
    socket.on("blockedUser", () => {
      showToastMessage("Your account has been blocked.");
  ...
  const uploadChatPic = () => {
    ...
    SharePicChat(formData).then((res) => {
      if (res) {
        let url = res.url;
  ...
  const deleteSelectedUsers = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        deleteSingleChat(selectedUsersId, userId).then((res) => {
          if (res) {
            updateSingleChat();
            navigate("/chatBox");
          }
  ...
  const clearSingleChat = (clearChatId) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        Swal.fire("Cleared!", "The chat has been cleared.", "success").then(
          () => {
            updateSingleChat();
          }
        );
  ...
  const onEmojiClick = (emojiObject) => {
    ...
    const selectedEmoji =
      emojiObject.emoji || emojiObject.character || emojiObject.native;
  ...
  const onChangeBlockUser = (chatRoomId, userId) => {
    userTouserBlock(chatRoomId, userId).then((res) => {
      updateSingleChat();
    });
  ...
  const unBlockUser = (chatRoomId, userId) => {
    userTouserUnblock(chatRoomId, userId).then((res) => {
      updateSingleChat();
    });
  };
  ...
  return (
    <>
      ...
      {responsiveMd ? (
        <div>
          {videoCall && (
            <VideoCall receiver={receiverId} closeVideoCall={closeVideoCall} />
          )}
          {permissionForVideoCall && (
            <PermissionNotification
              response={permissionForVideoCall}
              onAccept={goforcall}
          ...
          {reportShow && (
            <ReportManagement
              ...
              onclose={reportManagemgentClose}
            />
          )}
          {notification && (
              ...
              <div className="flex border border-gray-300 rounded shadow-lg h-screen w-full ">
                  ...
                  <form className="py-2 px-2 bg-gray-100">
                    ...
                    <div class="relative">
                      ...
                      <input
                        type="search"
                        id="default-search"
                        class="block w-full p-4 ps-10 text-sm  border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500   "
                        placeholder="Search ..."
                        required
                      />
                      <button
                        ...
                        return (
                          ...
                          >
                            ...
                            >
                              ...
                              {msg?.all?.receiver == userId &&
                                isURL(msg?.all?.message) && (
                              ...
                              {msg?.all?.sender == userId &&
                                isURL(msg?.all?.message) && (
                                  <p className="text-grey-dark mt-1 pr-8 text-sm text-right">
                                    image
                                  </p>
                                )}
                              {msg?.all?.sender == userId &&
                  ...
                  <div>
                    <div class="absolute top-5 right-10 z-50">
                      <div
                        ...
                        className=" w-full max-w-xs p-4  bg-white rounded-lg shadow  text-gray-300"
                        role="alert"
                      >
                        <div
                          style={{
                        ...
                        <span
                          ...
                          onClick={() => onChangeSelectMessaeOption(true)}
                        >
                          select message
                        </span>
                        <br />
                        ...
                        <span
                          className="mb-1 text-sm  text-black  "
                          onClick={() =>
                            onChangeBlockUser(blockUnblockchatroomId, userId)
                          }
                        >
                          block
                        </span>
                ...
                {receiverId && (
                  <div className="w-2/3 border flex flex-col  relative ">
                      ...
                      <div className="flex">
                        ...
                        {messages && (
                          <div
                            className="ml-3"
                            onClick={() =>
                              clearSingleChat(messages[0].chatroom)
                            }
                          >
                            <ClearAllIcon />
                          </div>
                          ...
                          messages.map((mess, index) => {
                            if (mess?.receiver == userId) {
                              ...
                              if (mess?.sender == userId) {
                                ...
                                return (
                                  <div
                                    className="flex justify-end mb-2 pr-8"
                                    key={index}
                                  >
                                    {selectMessageOption && (
                                      <input
                    ...
                    {console.log(
                      "chatrooomm details chatroom details chatroom details chatroom details chatrooom detials chatroom details"
                    )}
                    ...
                    <div>
                      {showsendMessageInputBox == "delete" && (
                        <div
                          className="bg-white border border-gray-200 rounded-md shadow-md flex items-center justify-between px-3 py-2"
                        ...
                        >
                          ...
                          <div style={{ paddingLeft: "20px" }}>
                            <span>{messagesId.length + "selected"}</span>
                          </div>
                      ...
                      {showsendMessageInputBox == "false" && (
                        <div className="bg-gray-300 px-4 py-4 flex items-center">
                          <div style={{ display: "flex", gap: " 16px" }}>
                            ...
                            <div>
                              ...
                              >
                                <path
                                  fill="#263238"
                                  d="M12 3c.553 0 1 .447 1 1v7h7c.553 0 1 .447 1 1s-.447 1-1 1h-7v7c0 .553-.447 1-1 1s-1-.447-1-1v-7h-7c-.553 0-1-.447-1-1s.447-1 1-1h7v-7c0-.553.447-1 1-1z"
                                />
                              </svg>
                              <input
                                type="file"
                          ...
                          <div className="flex-1 mx-4">
                            <input
                              className="w-full border rounded focus:outline-none px-2 py-2"
                              onChange={textMessageSend}
                    ...
                    {chatroomDetails?.status == true && (
                      <button
                        onClick={() =>
                          unBlockUser(blockUnblockchatroomId, userId)
                        }
                        className="bg-white border-gray-200 rounded-md shadow-md flex items-center justify-center px-3 py-2"
                      >
                        UN BLOCK
                      </button>
                    )}
                ...
                {receiverId == undefined && (
                  <div className="w-2/3 border flex flex-col items-center justify-center  relative bg-gray-100">
                    <img
                      src="https://www.kindpng.com/picc/m/241-2411768_facebook-messenger-icon-white-hd-png-download.png"
                      alt="Facebook Messenger Icon White, HD Png Download@kindpng.com"
          ...
          {prevFile && (
            <div className="fixed inset-0 flex justify-center items-start md:items-center bg-gray-200 p-4">
              <div className="relative w-full max-w-4xl h-[80vh] bg-gray-200 rounded-lg overflow-hidden shadow-lg">
                <svg
                  className="w-8 h-8 absolute top-4 right-4 cursor-pointer text-gray-600 hover:text-gray-800"
                  aria-hidden="true"
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
                  viewBox="0 0 14 14"
                ...
                {prevLoading ? (
                  ...
                  >
                    <SendTwoToneIcon style={{ fontSize: "40px" }} />
                  </div>
                )}
              </div>
            </div>
              ...
              <div className="flex border border-gray-300 rounded shadow-lg h-screen w-full ">
                {!receiverId && (
                  <div className="w-full border flex flex-col ">
                    ...
                    <div className="bg-grey-lighter flex-1 overflow-auto">
                      {console.log(
                        "side message message sidemesasge sidemssage"
                      )}
                      {console.log(sideMessage)}
                      {sideMessage &&
                        sideMessage.map((msg, index) => {
