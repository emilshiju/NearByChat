  ...
  const fetchProfile = async () => {
    try {
      ...
      if (res.status) {
        ...
        {
          console.log(
            "resssssssssssssssssssss useriddddddddddddddddddddddddddddddddddddddddddd"
          );
        }
        {
        ...
        setImage(
          "https://res.cloudinary.com/dwqtoz0ig/image/upload/v1717488014/nearbychat/gsu4hmgkwyy286tmq1wp.png"
        );
      }
    } catch (error) {
  ...
  const onChangeUploadImageToFirebase = () => {
    ...
    uploadBytes(storageRef, imageFile).then((snapshot) => {
      ...
      getDownloadURL(snapshot.ref).then((downloadURL) => {
        api
          .patch("/uploadUserProfileImage", { imageUrl: downloadURL, userId })
          .then((res) => {
            if (res.data.status) {
              isLoading(false);
              onCloseModalOpen();
  ...
  const onChangedeleteImage = (index) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        api.post("/deleteProfileImage", { userId, index }).then((res) => {
          if (res.data.status) {
            fetchProfile();
          }
        });
        ...
        <div className="lg:w-8/12 lg:mx-auto mb-8">
          ...
          <div className="px-px md:px-3">
            ...
            <ul className="flex items-center justify-around md:justify-center space-x-12 uppercase tracking-widest font-semibold text-xs text-gray-600 border-t">
              ...
              <div className="ml-[800px]" onClick={onChangeUploadImage}>
                {" "}
                <AddIcon />
              </div>
              <input
          ...
          <div className="flex flex-wrap items-center max-w-4xl mx-auto p-4">
            {profileImage &&
              profileImage.map((imageSrc, index) => (
                <div
                  key={index}
                  ...
                  <div className="overlay bg-gray-300 bg-opacity-50 w-full h-full absolute left-0 top-0 flex justify-center items-center space-x-2 opacity-0 transition-opacity duration-300 group-hover:opacity-100">
                    <span className=" flex items-center space-x-2 text-white cursor-pointer hover:bg-gray-700 rounded-full p-1">
                      <i
                        ...
                        onClick={() => onChangedeleteImage(index)}
                      ></i>
                    </span>
                  </div>
                </div>
