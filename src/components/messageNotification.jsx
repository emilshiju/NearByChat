...
const NewMessageNotificatoin=({message,setIsOpen})=>{
  ...
  useEffect(() => {
    ...
    return (
        ...
        role="alert"
      >
         {message&&<img className="object-cover rounded-full"style={{width:'30px',height:'30px'}}  src={message.imageUrl} alt="" />}
        {message&&<span>{message.nickName}</span>}
       {message &&<div className="ps-4 text-sm font-normal">{message.message}</div>}
        <svg
