...
const ChatToRandomPepoles = () => {
  ...
  const componentChangeSkip = () => {
    socket.emit(
      "userTouserOnlyRandomChatComponentChange",
      userId,
      oppositeUserId.current
    );
  };
  ...
  const onSendMessage = () => {
    if (oppositeUserId.current) {
      socket.emit("userTouserLiveChatting", {
        userId: userId,
        receiver: oppositeUserId.current,
      ...
      setCurrentMessage("");
        ...
        toast.success("not connected to any user !", {
          position: "top-right",
          autoClose: 1000,
        });
    ...
    <>
     <ToastContainer />
      <div className="relative">
        <div
          className="bg-white rounded shadow-lg mt-5 mx-4 sm:mx-10 md:mx-20 lg:mx-20"
          style={{
            width: "100%",
            maxWidth: "1400px",
            height: "590px",
        ...
        >
          {isLoading ? (
            ...
            <h1 className="text-3xl  border-r-black">now connected</h1>
          )}
            ...
            allMessage.map((a, b) => {
              return (
                <div>
                  <br></br>
                  {a.userId == userId ? (
                    <h1 className="ml-4 text-blue-500 text-base md:text-xl">
                      YOU:
                      {a.message && (
                        <span className="text-black ml-0">{a.message}</span>
                      )}
                      {a.video && (
                        <video
                          className=" mt-4"
      ...
      <h1 className="ml-4 text-blue-500 text-base md:text-xl">YOU: <span className="text-black ml-1">first</span></h1> */}
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-white flex flex-col gap-2 px-4 py-2 md:flex-row md:items-center md:gap-4">
          <button
            className="bg-red-500 text-white text-lg md:text-3xl px-6 md:px-10 py-2 rounded"
            onClick={onSkipNextOne}
          >
            Skip
          </button>
          <div className="relative flex-grow">
            <input
              className="w-full h-12 border border-black bg-white border-opacity-40 rounded px-3 py-2 outline-none pl-12"
              type="text"
            ...
            <AttachFileIcon
              ...
              className="absolute top-1/2 transform -translate-y-1/2 left-[1100px] cursor-pointer text-gray-500"
            />
            <input
              type="file"
              ref={fileInputRef}
          ...
          <button
            className="bg-green-500 text-white text-lg md:text-3xl px-6 md:px-10 py-2 rounded"
            style={{
              cursor: oppositeUserId.current ? "pointer" : "not-allowed",
            }}
        ...
        {showEmoji && (
          <div className="absolute z-[100] top-20 left-0 right-0 mx-auto mt-10 ">
